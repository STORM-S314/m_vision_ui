...
from MyHandTracker import MyHandTracker
...
def main():
    ...
    while not done:
        ...
        frame, hands, gesture= tracker.next_frame()
        ...
        if len(hands) == 1:
            ...
            rotate_mode_count = 0
            single_handed = True
            thumb = np.array([hands[0].landmark[4].x,hands[0].landmark[4].y,hands[0].landmark[4].z])
            index = np.array([hands[0].landmark[8].x,hands[0].landmark[8].y,hands[0].landmark[8].z])
        ...
        scale = 0
        relative_finger_distance = current_finger_distance.calc_average()*400 - middle_finger_distance
